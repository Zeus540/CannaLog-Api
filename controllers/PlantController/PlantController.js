...
module.exports = {
  getPublic: async(req, res) => {
    ...
    */
    ...
    sql_public = `
    SELECT users.user_name,users.user_id, irrigation_types.irrigation_type, strains.strain_name, plants.plant_id, plants.plant_name,plants.cover_img,plants.cover_img_next_gen,plants.cover_thumbnail_next_gen,plants.cover_thumbnail,DATE_FORMAT(plants.creation_date, '%Y-%m-%dT%H:%i:%sZ') AS creation_date,plants.last_updated,plants.environment_id,environments.environment_name,COUNT(plant_likes.plant_like_id) AS likes,COUNT(plant_views.plant_view_id) AS views
    FROM users
    JOIN plants ON users.user_id = plants.user_id
    JOIN irrigation_types ON irrigation_types.irrigation_type_id = plants.irrigation_type
    JOIN strains ON strains.strain_id = plants.plant_strain
    ...
    plants.plant_name,
    plants.cover_img,
    plants.cover_img_next_gen,
    plants.cover_thumbnail,
    plants.cover_thumbnail_next_gen,
    ...
    `
      ...
      LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
      LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
      WHERE plants.public = 1 AND plants.creation_date < ?
      GROUP BY
      users.user_name,
      irrigation_types.irrigation_type,
      strains.strain_name,
   ...
   await db.query(sql_public,[`${key_sort}`], async(err, result_pagination, fields) => {
      if (err) {
        console.log(err)
      } else {
        let sql_pagination_total = `SELECT COUNT(*) AS total FROM plants WHERE plants.public = 1`
    
        await  db.query(sql_pagination_total, (err, result, fields) => {
            if (err) {
              console.log(err)
  ...
  searchPublic:async(req, res)=>{
      ...
      */
      ...
      if(req.query.search_qeury == ''){
        res.status(500).send("No search qeury found ")
      }else{
      ...
      if(key_sort == "undefined"){
        ...
        LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
        LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
        WHERE plants.public = 1 AND plants.creation_date < ? AND strain_name LIKE '%${req.query.search_qeury}%'
        GROUP BY
        users.user_name,
        irrigation_types.irrigation_type,
        strains.strain_name,
        ...
        LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
        LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
        WHERE plants.public = 1 AND strain_name LIKE '%${req.query.search_qeury}%'
        GROUP BY
        users.user_name,
        irrigation_types.irrigation_type,
        strains.strain_name,
     ...
     await db.query(sql_public,[`${key_sort}`], async(err, result_pagination, fields) => {
        if (err) {
          console.log(err)
            ...
            let sql_pagination_total = `SELECT COUNT(*) AS total FROM plants JOIN strains ON strains.strain_id = plants.plant_strain WHERE plants.public = 1 AND strain_name LIKE '%${req.query.search_qeury}%' `
  ...
  searchPublicSignedIn:async(req, res)=>{
    ...
    */
    ...
    if(key_sort == "undefined"){
      ...
      LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
      LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
      WHERE plants.public = 1 AND plants.creation_date < ? AND plants.user_id != ${req.user.user_id} AND strain_name LIKE '%${req.query.search_qeury}%'
      GROUP BY
      users.user_name,
      irrigation_types.irrigation_type,
      strains.strain_name,
      ...
      LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
      LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
      WHERE plants.public = 1 AND plants.user_id != ${req.user.user_id} AND strain_name LIKE '%${req.query.search_qeury}%'
      GROUP BY
      users.user_name,
      irrigation_types.irrigation_type,
      strains.strain_name,
  ...
  getPublicSignedIn: async(req, res) => {
    /* ...
    // #swagger.tags = ['Plants']
    ...
    */
    ...
    `
      ...
      LEFT JOIN plant_likes ON plants.plant_id = plant_likes.plant_id
      LEFT JOIN plant_views ON plants.plant_id = plant_views.plant_id
      WHERE plants.public = 1 AND plants.user_id != ? AND plants.creation_date < ? 
      GROUP BY
      users.user_name,
      irrigation_types.irrigation_type,
      strains.strain_name,
    ...
    await  db.query(sql,[req.user.user_id,`${key_sort}`], async(err, result_pagination, fields) => {
      if (err) {
        console.log(err)
      } else {
        
        let sql_pagination_total = `SELECT COUNT(*) AS total FROM plants WHERE plants.public = 1 AND plants.user_id != ?`
    
      await  db.query(sql_pagination_total, [req.user.user_id], (err, result, fields) => {
          if (err) {
            console.log(err)
  ...
  */
  ...
  getMyPlants: async(req, res) => {
    ...
    */
    ...
    if (key_sort == "undefined") {
      getMyPlants_sql = `
      SELECT users.user_name,users.user_id, irrigation_types.irrigation_type, strains.strain_name, plants.plant_id, plants.plant_name,plants.cover_img,plants.cover_img_next_gen,plants.cover_thumbnail,plants.cover_thumbnail_next_gen,DATE_FORMAT(plants.creation_date, '%Y-%m-%dT%H:%i:%sZ') AS creation_date,plants.last_updated,plants.environment_id,environments.environment_name,COUNT(plant_likes.plant_like_id) AS likes,COUNT(plant_views.plant_view_id) AS views
      FROM users
      JOIN plants ON users.user_id = plants.user_id
      JOIN irrigation_types ON irrigation_types.irrigation_type_id = plants.irrigation_type
      JOIN strains ON strains.strain_id = plants.plant_strain
  ...
  update_cover_image: (req, res) => {
    /* ...
      ...
      */
      ...
      cover_img_next_gen = ?, 
      cover_thumbnail = ?,
      cover_thumbnail_next_gen = ?
      WHERE plant_id = ?
      `
console.log("req.body.cover_img",req.body.cover_img)
console.log("req.body.cover_thumbnail",req.body.cover_thumbnail)

    db.query(sql, [req.body.cover_img,req.body.cover_img_next_gen,req.body.cover_thumbnail,req.body.cover_thumbnail_next_gen,req.params.plant_id], (err, result, fields) => {
      if (err) {
        console.log(err)
      } else {
        res.send(result[0])
  ...
  get_detailed_plant_info_public: (req, res)=>{
   /* ...
    ...
    let getMyPlants_sql = `
    SELECT users.user_name,users.user_id, irrigation_types.irrigation_type, strains.strain_name, plants.plant_id, plants.plant_name,plants.cover_img,plants.cover_img_next_gen,plants.cover_thumbnail,plants.cover_thumbnail_next_gen,plants.public,DATE_FORMAT(plants.creation_date, '%Y-%m-%dT%H:%i:%sZ') AS creation_date,plants.last_updated,plants.environment_id,environments.environment_name,COUNT(plant_likes.plant_like_id) AS likes,COUNT(plant_views.plant_view_id) AS views
    FROM users
    JOIN plants ON users.user_id = plants.user_id
    JOIN irrigation_types ON irrigation_types.irrigation_type_id = plants.irrigation_type
    JOIN strains ON strains.strain_id = plants.plant_strain
